import { 
  ResponseGenerator as IResponseGenerator, 
  SkillResult, 
  ConversationContext 
} from '../types';
...
export class ResponseGenerator implements IResponseGenerator {
  ...
  public generateErrorResponse(error: { type: 'network' | 'permission' | 'context' | 'unknown'; recoverable: boolean; user_message: string; retry_suggested: boolean }, language: 'ar' | 'tr' | 'en'): string {
    const messages = {
      ar: {
  ...
  private generateErrorFromResult(result: SkillResult, templates: any): string {
  ...
  private determineActionType(result: SkillResult): string {
    if (result.data?.query && result.data?.results) return 'search';
    ...
    if (result.data?.action === 'dev_fix') return 'dev_fix';
    if (result.data?.action === 'dev_fix_preview') return 'dev_fix_preview';
    if (result.data?.action === 'open_file') return 'open_file';
    if (result.data?.action === 'read_file') return 'read_file';
    if (result.data?.action === 'summarize_logs') return 'summarize_logs';
    
    return 'general';
  }

  private determineErrorType(result: SkillResult): string {
    const error = result.error?.toLowerCase() || '';
    
    if (error.includes('permission') || error.includes('access')) return 'permission';
    if (error.includes('not found') || error.includes('no results')) return 'not_found';
    if (error.includes('context') || error.includes('missing')) return 'context';
    if (error.includes('network') || error.includes('connection')) return 'network';
    
    return 'general';
  }

  private detectLanguage(context: ConversationContext): 'ar' | 'tr' | 'en' {
  ...
  private detectLanguageFromText(text: string): 'ar' | 'tr' | 'en' {
    const arabicChars = /[\u0600-\u06FF]/;
