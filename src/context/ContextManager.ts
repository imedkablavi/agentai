...
export class ContextManager implements IContextManager {
  ...
  isFollowUpRequired(intent: Intent): boolean {
    ...
    const isFollowUpIntent = keywords.some(keyword => 
      intent.raw_text.toLowerCase().includes(keyword.toLowerCase())
    );

    const isIndexReference = /^\d+$/.test(intent.raw_text.trim());
    const isSelectionIntent = ['select', 'اختر', 'seç'].some(word => 
      intent.raw_text.toLowerCase().includes(word.toLowerCase())
    );
