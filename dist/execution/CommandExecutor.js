...
class CommandExecutor {
    ...
    async execOpenFile(command, lang) {
    ...
    async execReadFile(command, lang) {
        const target = command.target || '';
        if (!target) {
            return { success: false, error_detail: this.error('context', true, this.msg(lang, 'file_not_specified'), false) };
        }
    ...
    async execSummarizeLogs(command, lang) {
        ...
        for (const target of candidates) {
            ...
            const summary = `${this.msg(lang, 'log_file')}: ${target}\n` +
    ...
    async execDevInspect(command, lang) {
...
---\n`;
    ...
    async execDevTest(command, lang) {
        const start = Date.now();
    ...
    async execDevFix(command, lang) {
        ...
        const target = command.target || '';
        if (!target)
            return { success: false, error_detail: this.error('context', true, this.msg(lang, 'path_not_specified'), false) };
        const { content, error } = await this.fsSafety.readFile(target);
        if (error)
            return { success: false, error_detail: this.error('permission', true, error, false) };
    ...
    riskLevelText(level, lang) {
        if (lang === 'ar')
            return level === 'high' ? 'عالي 🔴' : level === 'medium' ? 'متوسط 🟡' : 'منخفض 🟢';
        if (lang === 'tr')
            return level === 'high' ? 'Yüksek 🔴' : level === 'medium' ? 'Orta 🟡' : 'Düşük 🟢';
        return level === 'high' ? 'High 🔴' : level === 'medium' ? 'Medium 🟡' : 'Low 🟢';
    }
    confidenceLevelText(score, lang) {
        const level = score >= 0.7 ? 'high' : score >= 0.4 ? 'medium' : 'low';
        if (lang === 'ar')
            return level === 'high' ? 'عالية 🟢' : level === 'medium' ? 'متوسطة 🟡' : 'منخفضة 🔴';
        if (lang === 'tr')
            return level === 'high' ? 'Yüksek 🟢' : level === 'medium' ? 'Orta 🟡' : 'Düşük 🔴';
        return level === 'high' ? 'High 🟢' : level === 'medium' ? 'Medium 🟡' : 'Low 🔴';
    }
